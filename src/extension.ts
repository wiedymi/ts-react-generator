  ...
  const createComponent = (uri, suffix: string = "") => {
    ...
    enterComponentNameDialog$
      .concatMap(val => {
        ...
        const container = suffix.indexOf("redux") !== -1;

        return Observable.forkJoin(
          FileHelper.createComponent(componentDir, componentName, suffix),
          FileHelper.createIndexFile(componentDir, componentName, container),
          FileHelper.createCSS(componentDir, componentName),
          FileHelper.createContainer(componentDir, componentName, container)
        );
      })
